...
DATE <- seq(as.Date("2020-03-01"),as.Date("2020-03-02"),"days")
(parameter0 = log(c(0.277347463,	0.864681623,	1.131945414,	0.197168164,	0.514870368,	0.751606605)))
#(parameter0 = log(c(9.882186736,	0.012934697,	0.024889866,	68681974.96,	0.009584841,	272.0859302)))
#(parameter0 = log(c(29.2104801,	0.29530499,	0.046327112,	18338651.35,	0.014489231,	462.1392219)))
#i=1
for (i in 1:length(DATE)){
  ...
  Opt <- try(optim(parameter0,
               RSS,
               method = "BFGS"))#,control = list(maxit = 10000),hessian = F)
...
write.csv(save,"results0121_3.csv")
